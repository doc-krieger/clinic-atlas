...
**Directives:**
- **Communication:** Keep responses action-oriented and concise. Skip lengthy preambles and get straight to the work. When reporting results, use brief summaries unless asked for detail.
- **Decisions:** Present a recommended option and proceed unless the decision has significant architectural consequences. For high-stakes choices, present options concisely with a clear recommendation rather than open-ended lists.
- **Explanations:** Provide working code with brief explanations of non-obvious decisions and patterns. Not verbose walkthroughs, but enough to build understanding -- this developer is self-taught and appreciates knowing *why*, not just *what*.
- **Debugging:** When encountering errors, diagnose and fix them directly. Provide a brief one-line explanation of the root cause alongside the fix, but do not ask clarifying questions before attempting a solution.
- **UX Philosophy:** Prioritize working functionality over visual polish. When building UI components, use sensible defaults from the component library without asking about styling preferences unless the task is explicitly design-focused.
- **Vendor Choices:** Recommend the best-fit library or tool and proceed. Do not present comparison tables unless explicitly asked. When suggesting a dependency, briefly justify the choice in one sentence.
- **Frustrations:** Follow instructions completely -- do not leave placeholders, skip steps, or cut corners. Always run tests and checks before declaring work done. If you cannot fully complete a request, explicitly state what remains rather than silently delivering partial work.
- **Learning:** This developer is self-taught (not a programmer by training) and learns by doing. Provide working code with brief explanations of *why* the approach works -- not lengthy tutorials, but enough context to build understanding. When introducing new patterns, add a sentence or two explaining the reasoning.
<!-- GSD:profile-end -->
