...
**Confidence:** HIGH (most choices verified against current official docs and Context7)
...
```bash
# Backend (uv)
uv add fastapi==0.135.3 sqlmodel httpx pydantic-settings pymupdf4llm trafilatura python-frontmatter pyyaml alembic
uv add "litellm>=1.83.0"   # IMPORTANT: never install 1.82.7 or 1.82.8
uv add --dev ruff

# Frontend (pnpm)
pnpm add react-markdown remark-gfm react-syntax-highlighter @tanstack/react-query zustand
...
**Markdown knowledge base on disk:**
...
The project requires web search scoped to trusted medical domains. The recommended approach is a **self-hosted SearXNG instance** added to the Docker Compose stack.
...
async with httpx.AsyncClient() as client:
    resp = await client.get(
        ...
        params={"q": f"site:{domain} {query}", "format": "json"},
    )
    results = resp.json()["results"]
```

The source registry YAML maps trusted source names to domains; the agent builds `site:` queries per domain.
...
---
*Stack research for: Clinic Atlas — self-hosted clinical knowledge assistant*
*Researched: 2026-04-06*
